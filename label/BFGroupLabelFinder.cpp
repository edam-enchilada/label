/* ***** BEGIN LICENSE BLOCK *****
 * Version: MPL 1.1/GPL 2.0/LGPL 2.1
 *
 * The contents of this file are subject to the Mozilla Public License Version
 * 1.1 (the "License"); you may not use this file except in compliance with
 * the License. You may obtain a copy of the License at
 * http://www.mozilla.org/MPL/
 *
 * Software distributed under the License is distributed on an "AS IS" basis,
 * WITHOUT WARRANTY OF ANY KIND, either express or implied. See the License
 * for the specific language governing rights and limitations under the
 * License.
 *
  *
 * The Initial Developer of the Original Code is
 * The EDAM Project at University of Wisconsin, Madison
 *
 * Contributor(s):
 *  Lei Chen    lchen5@gmail.com
 *
 * Alternatively, the contents of this file may be used under the terms of
 * either the GNU General Public License Version 2 or later (the "GPL"), or
 * the GNU Lesser General Public License Version 2.1 or later (the "LGPL"),
 * in which case the provisions of the GPL or the LGPL are applicable instead
 * of those above. If you wish to allow use of your version of this file only
 * under the terms of either the GPL or the LGPL, and not to allow others to
 * use your version of this file under the terms of the MPL, indicate your
 * decision by deleting the provisions above and replace them with the notice
 * and other provisions required by the GPL or the LGPL. If you do not delete
 * the provisions above, a recipient may use your version of this file under
 * the terms of any one of the MPL, the GPL or the LGPL.
 *
 * ***** END LICENSE BLOCK ***** */
 
 #include <SimpleGroupLabelFinder.h>
#include <BFGroupLabelFinder.h>
#include <BranchAndBoundLabelFinder.h>

BFGroupLabelFinder::~BFGroupLabelFinder() {
  clearCandidateSet();  
  delete pLabeler;
  pLabeler= NULL;
}

void BFGroupLabelFinder::init() {
  cout << "Init branch and bound algorithm" << endl;

  pLabeler->setTile(pTile);
  pLabeler->setErrorBound(error);  

  vector<Spectrum *>::iterator it = spectra.begin();
  while ((it != spectra.end())) {
	Spectrum *sp = *it;
	//	cout << " BFGF :" << endl;
	//	sp->dump();
	pLabeler->setSpectrum(sp);	
	
	pLabeler->init();
	DiscreteLabel *label = pLabeler->getNext();
	while (label != NULL) {
	  CandidateIterator cit = candidates.find(label);
	  if (cit == candidates.end()) 
		candidates[label] = 1;
	  else
		(*cit).second++;

	  label = pLabeler->getNext();
	}

	it++;
  }

  currSolution = candidates.begin();

  int numSigs = (*currSolution).first->getSize();
  int data[10000];
  memset(data, 0, sizeof(int) * numSigs);
  DiscreteLabel *aLabel = new DiscreteLabel(numSigs, data);
  //  cout << "BFGLF - The frequency of null label" << endl;
  //  cout << candidates[aLabel] << endl;
  delete aLabel;
}

DiscreteLabel *BFGroupLabelFinder::getNext() {
  int spcNum = spectra.size();
  int minVote = (int)(support * spcNum + 1);
  while ((currSolution != candidates.end()) &&
		 ((*currSolution).second < minVote)) {
	currSolution++;
  }

  if (currSolution != candidates.end()) {
	DiscreteLabel *cp = (*currSolution).first;
	currSolution++;
	DiscreteLabel *result = new DiscreteLabel(*cp);
	return result;
  } else
	return NULL;
}
	
void BFGroupLabelFinder::clearCandidateSet() {
  for (CandidateIterator it = candidates.begin();
	   it != candidates.end(); it++) {
	delete (*it).first;
  }
  candidates.clear();
}

int * BFGroupLabelFinder::getHistogram() {
  int spcNum = spectra.size();
  int *data = new int[spcNum+1];
  memset(data, 0, sizeof(int) * (spcNum+1));

  for (CandidateIterator it = candidates.begin();
	   it != candidates.end(); it++) 
	data[(*it).second]++;

  return data;
}
