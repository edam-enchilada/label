 ...
 * ***** END LICENSE BLOCK ***** */
 ...
 #include <NewCrawlLabelFinder.h>

NewCrawlLabelFinder::~NewCrawlLabelFinder() {
  clearQueue();
  delete [] possible;
}

// Initialize the crawl labeling algorithm
// We basically filter out those signature
// which is impossible to reside in the 
// particle
...
void CrawlLabelFinder::init() {
  ...
  if (label != NULL) {
	...
	if (err < this->error) {
	  ...
	  CrawlLabelEntry *e = new CrawlLabelEntry(pl);
	  queue.push_back(e);
...
DiscreteLabel *CrawlLabelFinder::getNext() {
  if (queue.empty()) {
	...
	list<CrawlLabelEntry *>::iterator it = queue.begin();
	CrawlLabelEntry *entry = *it;
	queue.erase(it);
	DiscreteLabel *result = entry->label;
...
void NewCrawlLabelFinder::clearQueue() {
  for (list<CrawlLabelEntry *>::iterator it = queue.begin();
	   it != queue.end();
	   ++it) {
	CrawlLabelEntry *e = *it;
	delete e;
  }
