...
#ifndef BRANCH_AND_BOUND_GROUP_LABEL_FINDER_H
#define BRANCH_AND_BOUND_GROUP_LABEL_FINDER_H
...
class BBGroupLabelFinder : public DiscreteGroupLabelFinder {
  public :
	BBGroupLabelFinder(const SignatureDatabase *sd,
						   const Tile *tile = NULL,
						   double err = 0,
						   double support = 1) :
	DiscreteGroupLabelFinder(sd, tile, err, support) {
	qLabeler = new ManhattenDistanceLabelFinder(*sd);
	compensateCounter = 0;
	voteMark = NULL;
	minor = NULL;
  };
  ...
  int getSolveCount() { return qLabeler->getSolveCount(); };
  void clearSolveCount() { qLabeler->clearSolveCount(); };

  long getCompensateCount() { return compensateCounter; };
  void setCompensateCount(long c) { compensateCounter = c; }
